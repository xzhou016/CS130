...
GLuint loadTarga(const char* filename)
{
    ...
    fclose(file);
    //std::cout << "data type RGBA" << typeid(GL_RGBA).name() << " RGB" << typeid(GL_RGB).name() << std::endl;
    ...
    glTexImage2D (GL_TEXTURE_2D, 0, (bytesPerPixel == 3) ? GL_RGB : GL_RGBA, width, height, 0, (bytesPerPixel == 3) ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE, data);
...
void application::init_event()
{
    ...
    //texture[1] = loadTarga("monkey_ambient_occlusion.tga");
